...
class Encoder(nn.Module):#Transformer过程中不改变hidden_states的结构
    '''论文中实际的Transformer结构，从中引出skip-connection到decoder
    '''
    def __init__(self, config, vis):
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.up_relu = nn.ReLU()#是否使用ReLU需要自己手动注释

    def forward(self, x, skip=None):
        x = self.up(x)#上采样h,w先扩展一倍b1(3,512,28,28) b3(3,128,112,112)
        x = self.up_relu(x)#是否使用ReLU需要自己手动注释
        #x = self.norm(x)
        if skip is not None:
...
class DecoderCup_mix(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        head_channels = 512
...
class VisionTransformer_New2(nn.Module):
    '''
    此为自己对论文结构的修改形成的网络
    结合上采样与skip-connection建立的模型
    '''
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(VisionTransformer_New2, self).__init__()
        ...
        self.decoder = DecoderCup_mix(config)
